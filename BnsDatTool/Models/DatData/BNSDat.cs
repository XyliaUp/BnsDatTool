...
using static Xylia.bns.Third.MySpport;
...
namespace Xylia.bns.Modules.DataFormat.Dat
{
	public sealed class BNSDat
	{
		...
		public BNSDat(string DatPath, bool? Is64 = null, byte[] AES = null)
		{
			...
			this.Bit64 = Is64 ?? DatPath.Judge64Bit();

			this.Read(DatPath, this.Bit64);
		}
		#endregion
		...
		{
			if (!File.Exists(FileName)) throw new ArgumentException("请选择文件路径");

			#region 读取基础数据
			using var br = new BinaryReader(new FileStream(FileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite));
			this.Signature = br.ReadBytes(8);
			this.Version = br.ReadUInt32();
			this.Unknown_001 = br.ReadBytes(5);
			this.FileDataSizePacked = Is64 ? (int)br.ReadInt64() : br.ReadInt32();
			var FileCount = Is64 ? (int)br.ReadInt64() : br.ReadInt32();
			this.IsCompressed = br.ReadBoolean();
			this.IsEncrypted = br.ReadBoolean();

			// Update 200429
			if (this.Version == 3)
			{
				this.Auth = br.ReadBytes(128);
			}


			this.Unknown_002 = br.ReadBytes(62);

			this.FileTableSizePacked = Is64 ? (int)br.ReadInt64() : br.ReadInt32();
			this.FileTableSizeUnpacked = Is64 ? (int)br.ReadInt64() : br.ReadInt32();

			byte[] FileTablePacked = br.ReadBytes(this.FileTableSizePacked);

			//不要相信数值，请读取当前流位置
			this.OffsetGlobal = Is64 ? (int)br.ReadInt64() : br.ReadInt32();
			this.OffsetGlobal = (int)br.BaseStream.Position;
			#endregion

			#region 读取文件描述信息
			byte[] FileTableUnpacked = Unpack(FileTablePacked, this.FileTableSizePacked, this.FileTableSizePacked, this.FileTableSizeUnpacked, this.IsEncrypted, this.IsCompressed, this.KeyInfo);
			...
			//读取文件描述信息
			this.FileTableList = new List<BPKG_FTE>();
			for (int i = 0; i < FileCount; i++)
				FileTableList.Add(new BPKG_FTE(br2, Is64, this.OffsetGlobal) { KeyInfo = this.KeyInfo });
		...
		public static void Pack(PackParam param)
		{
			...
			Parallel.ForEach(list, _file =>
			{
				...
				if (_file.FilePath.EndsWith(".xml") || _file.FilePath.EndsWith(".x16")) tmp = BXML_CONTENT.Convert(fis, BXML_TYPE.BXML_BINARY);
				else fis.CopyTo(tmp);
				...
				_file.Data = BNSDat.Pack(originalData, _file.FileDataSizeUnpacked, out _file.FileDataSizeSheared, out _file.FileDataSizeStored, _file.IsEncrypted, _file.IsCompressed, Level, param.Aes);
			...
			var buffer_packed = BNSDat.Pack(buffer_unpacked, FileTableSizeUnpacked, out FileTableSizeSheared, out FileTableSizePacked, IsEncrypted, IsCompressed, Level, param.Aes);
		...
		/// <param name="sizeDecompressed">压缩后数据长度</param>
		/// <returns></returns>
		...
		/// </summary>
		/// <param name="buffer"></param>
		/// <param name="sizeDecompressed">原始长度</param>
		...
		/// </summary>
		/// <param name="buffer"></param>
		/// <param name="sizeStored"></param>
		/// <param name="sizeSheared"></param>
		/// <param name="sizeUnpacked"></param>
		/// <param name="isEncrypted">指明数据是加密的</param>
		/// <param name="isCompressed">指明数据是压缩的</param>
		...
		/// </summary>
		/// <param name="buffer"></param>
		/// <param name="sizeUnpacked">解包大小</param>
		/// <param name="sizeSheared">裁剪大小</param>
		/// <param name="sizeStored">存储大小</param>
