...
namespace Xylia.bns.Modules.DataFormat.ZoneData.RegionData
{
	public class RegionFile
	{
		...
		public void Read(BinaryReader br)
		{
			...
			if (this.Version != 21) Console.WriteLine("地形版本号不为21");

			this.FileSize = br.ReadInt64();
			this.RegionID = br.ReadInt32();
			this.XRange = br.ReadInt16();
			this.YRange = br.ReadInt16();
			...
			Console.WriteLine($"Xmin={this.Xmin} | Xmax={this.Xmax} | Ymin={this.Ymin} | Ymax={this.Ymax} ({XRange * YRange} 区块)");
		...
		{
			//重新计算长度
			this.XRange = (short)(Xmax - Xmin + 1);
			this.YRange = (short)(Ymax - Ymin + 1);
		...
		public void OutputTest(string Path)
		{
			...
			Region.SetAttribute("id", this.RegionID.ToString());
			...
			#region Area
			foreach (var area in this.RegionArea)
			{
		...
		{
			#region 初始化
			this.RegionArea = new List<RegionArea>();

			XmlDocument XmlDoc = new();
			XmlDoc.Load(Path);

			var Region = XmlDoc.SelectSingleNode("table/region");
			this.Xmin = short.Parse(Region.Attributes["Xmin"].Value);
			this.Xmax = short.Parse(Region.Attributes["Xmax"].Value);
			this.Ymin = short.Parse(Region.Attributes["Ymin"].Value);
			this.Ymax = short.Parse(Region.Attributes["Ymax"].Value);
			#endregion
