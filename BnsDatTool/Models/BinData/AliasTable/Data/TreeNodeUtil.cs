	...
	public static class TreeNodeUtil
	{
		...
		private static void CreateTree(this string INFO, AliasInfo Node, TreeNode Tree = null)
		{
			...
			if (!string.IsNullOrWhiteSpace(Group))
			{
				...
				CreateTree(INFO.RemovePrefixString(Group), Node, CurTree);
			}
			else
			{
				//树最终截止到此
		...
		public static void CheckTree(this TreeNode Tree)
		{
			...
			{
				foreach (var Children in Tree) CheckTree(Children);
			}
		}

		public static List<NameTableEntry> CreatCases(this TreeNode TreeNode)
		{
			var Result = CreatCases(TreeNode, new List<int>());
		...
		/// <param name="Info"></param>
		/// <returns></returns>
		public static string MyTake(this string Info) => new(MyTakeToChars(Info));
