...
namespace Xylia.bns.Modules.DataFormat.Analyse.Load.Config.Table
{
	public static class TableRead
	{
		public static void TableConfigRead(this List<TableInfo> TableInfos, XmlNode XNode, ConfigParam ConfigInfo)
		{
			...
			#region 读取部分list信息 
			var Rules = xp.Attributes["rule"].GetListRules(out var msg);    //规则
			...
			if (CheckSize is null)
			{
				...
				//throw new Exception($"[{ ListSignal }] 配置文件初始化失败", ex);
			}
			#endregion

			#region 读取记录器信息
			TableInfo RecordInfo = GetRecordInfo(XNode.ChildNodes.OfType<XmlElement>(), ListSignal, ConfigInfo);
		...
		public static TableInfo GetRecordInfo(this IEnumerable<XmlElement> XmlElements, string ListSignal, ConfigParam ConfigInfo = null)
		{
			...
			foreach (XmlElement Child in XmlElements)
			{
				...
				var Filters = RecordNode.Attributes["filter"].GetFilters(TableInfo.TypeInfo);  //初始化过滤信息
				...
				if (Type == AttributeType.TNone)
				{
					if (RecordNode.ContainAttribute("struct", out string StructInfo))
					{
						...
						foreach (var r in FStruct)
						{
							...
							if (rc.OutCond != null)
							{
								rc.OutCond = (Condition.Condition)r.OutCond.Clone();
			...
			if (ListSignal != "#struct")
			{
				...
				if (DebugSwitch.Status) TableInfo.RecordTables.ShowInfo();
			}
