...
namespace Xylia.bns.Modules.DataFormat.Analyse.Record
{
	public sealed class RecordDef : ICloneable
	{
		...
		/// 当前结束索引
		/// </summary>
		public int EndIndex => this.Offset + (this.Size * Repeat);
		...
		/// <returns></returns>
		public int GetOffset(int Index = 0) => this.Offset + this.Size * Index;

		public string GetAlias(int Index = 0) => this.Alias + (Repeat == 1 ? null : "-" + Index);
		#endregion
		...
		/// 服务端输入条件
		/// </summary>
		public Condition.Condition ValidCond;
		...
		/// </summary>
		public bool UseRef => this.RefInfo?.RefTable != null;
		#endregion
		...
		/// 是否有枚举
		/// </summary>
		public bool HasSeq => this.Seq != null;
		#endregion
