...
namespace Xylia.bns.Modules.DataFormat.Analyse.Struct
{
	...
	public class DefaultInfo
	{
		...
		public DefaultInfo(string Value, string CorrectVal)
		{
			...
			this.ConditionValue = CorrectVal?.Trim();
		...
		public void Load(string Info)
		{
			try
			{
				...
				if (Info.StartsWith("$"))
				{
					...
					if (System.Enum.TryParse<Symbol>(Symbol, true, out var result)) this.Symbol = result;
					else
					{
						this.Symbol = Struct.Symbol.Copy;
						if (string.IsNullOrWhiteSpace(this.SymbolContent))
				...
				else if (Info.Contains("{") && Info.Contains("}"))
				{
					...
					this.SymbolContent = regex.Match(Info).Groups[1].Value;      //获取原文信息

					Extra = regex.Replace(" " + Info + " ", "$").Split('$').ToList();
					this.Symbol = Symbol.Text;
				}

				this.SymbolContent = this.SymbolContent.Trim();
			}
