...
namespace Xylia.bns.Modules.DataFormat.Analyse.Seq
{
	public class SeqInfo<SeqCell> : List<SeqCell>, ICloneable where SeqCell : Seq.SeqCell
	{
		...
		public new void Add(SeqCell item)
		{
			...
			{
				if (!this.ht_alias.ContainsKey(item.Alias)) this.ht_alias.Add(item.Alias, item);
				else LogWriter.WriteLine($"== CRASH == 枚举初始化错误，别名重复 ({this.Name} => { item.Alias })");
			}

			if (!this.ht_id.ContainsKey(item.Key)) this.ht_id.Add(item.Key, item);
			else LogWriter.WriteLine($"== CRASH == 枚举初始化错误，主键重复 [{this.Name} => {item.Alias} ({item.Key})]");
		...
		{
			if (this.ht_alias.ContainsKey(item.Alias)) this.ht_alias.Remove(item.Alias);
			if (this.ht_id.ContainsKey(item.Key)) this.ht_id.Remove(item.Key);
		...
		public bool ContainsAlias(string alias) => this.ht_alias.ContainsKey(alias);

		public bool ContainsKey(short key) => this.ht_id.ContainsKey(key);

		public SeqCell GetCell(string Alias) => this.ht_alias.ContainsKey(Alias) ? this.ht_alias[Alias] as SeqCell : null;

		public SeqCell GetCell(short Key) => this.ht_id.ContainsKey(Key) ? this.ht_id[Key] as SeqCell : null;
		...
		{
			if (this.Count == 0) return Key.ToString();

			#region 返回枚举查询结果
			var SeqCell = this.GetCell(Key);
			...
			if (Key != 0)
			{
				...
				{
					string ErrorMsg = $"枚举 { this.Name } 中不包含: { Key }";
		...
		{
			var KeyInfo = this.GetCell(Alias);
			if (KeyInfo != null)
			{
				Key = KeyInfo.Key;
				return true;
			...
			else if (ShowMsg)
			{
				...
				if (!MsgCache.ContainsKey(ErrorKEY))
				{
					string ErrorMsg = $"获取枚举信息失败 ({ this.Name } => { Alias })";
					Console.WriteLine(ErrorMsg);
					LogWriter.WriteLine(ErrorMsg);
